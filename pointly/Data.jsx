...
const NET_CONFIG = {
  ...
  "9mobile":{ bg:"#111B27", color:"#FFC107", text:"9"      },
};

function getNetConfig(networkName) {
  const key = networkName?.toLowerCase().replace(/\s/g, "");
  return NET_CONFIG[key] || { bg:"#6A00DF", color:"#fff", text: networkName?.charAt(0) || "N" };
}
...
const STYLES = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dt-contact-btn { background:none; border:none; cursor:pointer; color:#9ca3af; display:flex; align-items:center; padding:0; }
  .dt-error-msg { font-size:12px; color:#EF4444; font-weight:500; margin-top:6px; }

  .dt-tabs { display:flex; background:#fff; border-radius:14px; padding:4px; gap:4px; overflow-x:auto; }
  ...
  .dt-plans-label { font-size:14px; font-weight:700; color:#111B27; margin:16px 0 10px; }
  ...
  .dt-banner { background:#F0ECF9; border-radius:14px; padding:14px 16px; display:flex; align-items:center; gap:12px; cursor:pointer; }
  ...
  .dt-continue-btn { width:100%; height:58px; background:linear-gradient(135deg,#6A00DF,#8B3DFF); border:none; border-radius:16px; font-family:'Poppins',sans-serif; font-size:16px; font-weight:800; color:#fff; cursor:pointer; box-shadow:0 8px 24px rgba(106,0,223,.3); transition:transform .15s, opacity .15s; }
  .dt-continue-btn:active { transform:scale(.97); }
  .dt-continue-btn:disabled { opacity:.5; cursor:not-allowed; }
...
// Extract plans array from any response shape the API might return
function extractPlans(data) {
  ...
  // { data: [...] }  — same flat shape as /networks
...
export default function Data() {
  ...
  useEffect(() => {
    ...
    // FIX: use network_id (numeric ID) not search (string name)
    const url = `${POINTLY_API_BASE}/vtu/data-plans?network_id=${network.id}&limit=200`;

    fetch(url, { headers: { "X-API-Key": POINTLY_API_KEY } })
      .then(r => r.json())
      .then(data => {
      ...
      .catch(() => setPlansError("Failed to load data plans."))
    ...
    <>
      ...
      <div className="dt-root">
        <div className="dt-scroll">
            ...
            {netsLoading ? (
              ...
              <div className="dt-networks">
                {networks.map(n => {
                  ...
                  return (
                    <div className="dt-net-item" key={n.id}
                      onClick={() => { setNetwork(n); setPlan(null); }}>
          ...
          <div className="dt-section">
            ...
            <div className={`dt-input-wrap${errors.phone ? " error" : ""}`}>
              ...
              <button className="dt-contact-btn"><PersonIcon /></button>
            </div>
          ...
          <div className="dt-section">
            ...
            {plansLoading ? (
              ...
              <>
                  ...
                  <div className="dt-tabs">
                    {categories.map(cat => (
                      <button key={cat}
                        ...
                        onClick={() => { setActiveTab(cat); setPlan(null); setErrors(e=>({...e,plan:""})); }}>
                        {cat}
                      </button>
                    ))}
                  </div>
                ...
                <div className="dt-plans-label">{activeTab} Plans</div>
        ...
        <div className="dt-bottom">
          <button className="dt-continue-btn" onClick={handleContinue} disabled={!plan || plansLoading}>
